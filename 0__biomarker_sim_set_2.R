#!/usr/bin/env Rscript
#
# Simulate 100 studies with different standard deviations
# Cohen's d and group size fixed at 1.25 and 400, respectively 
# Save histograms and ROC-curves for the first iteration
#
...
# baseline mean + effect size + sample size
mean_control <- 100
N_train <- 400
...
plots <- vector( mode = 'list', length = length( SDs ) )
plots_roc <- vector( mode = 'list', length = length( SDs ) )
counter <- 1
counter_roc <- 1
...
for( sd in SDs )
  ...
  for( iter in 1:niter )
    {
    # mean intervention group (fixed)
    mean_intervention <- mean_control + 15 
    
    # cohen's d
    cohens_d <- ( mean_intervention - mean_control ) / sd
    ...
    if( iter == 1 )
      ...
      plots[[ counter ]] <- list( N_train = N_train, sd = sd, cohens_d = cohens_d, p = p )
      counter <- counter + 1
      }
    ...
    if( iter == 1 ) 
      ...
      # add to container
      plots_roc[[ counter_roc ]] <- list( N_train = N_train, sd = sd, cohens_d = cohens_d, p = p_roc )
...
p_comb_hist_roc_sd <- plot_grid( 
  
  # plots at SD 10
  plots[[ 1 ]]$p + 
    theme( legend.position = "none" ) +
    theme( axis.title = element_text( size = 12, colour = "black" ), axis.text = element_text( size = 11, colour = "black" ) ) +
    coord_cartesian( xlim = c( 0, 220 ), ylim = c( 0, 65 ) ) +
    annotate( "text", label = paste0( "SD: ", plots[[ 1 ]]$sd ), x = 23, y = 56, size = 4, colour = "gray30" )+
    annotate( "text", label = paste0( "Cohen's d: ", format( plots[[ 1 ]]$cohens_d, nsmall = 2 ) ), x = 42, y = 52, size = 4, colour = "gray30" ),
  ...
  plots[[ 2 ]]$p + 
    ...
    theme( axis.title = element_text( size = 12, colour = "black" ), axis.text = element_text( size = 11, colour = "black" ) ) +
    coord_cartesian( xlim = c( 0, 220 ), ylim = c( 0, 65 ) ) +
    annotate( "text", label = paste0( "SD: ", plots[[ 2 ]]$sd ), x = 23, y = 56, size = 4, colour = "gray30" )+
    annotate( "text", label = paste0( "Cohen's d: ", format( plots[[ 2 ]]$cohens_d, nsmall = 2 ) ), x = 42, y = 52, size = 4, colour = "gray30" ),
  
  plots_roc[[ 2 ]]$p + 
    theme( legend.position = "none" ) +
    theme( axis.title = element_text( size = 12, colour = "black" ), axis.text = element_text( size = 11, colour = "black" ) ) +
    annotate( "text", label = paste0( "AUC: ", format( round( roc_ci[ 2, 2 ], digits = 2 ), nsmall = 2 ),
                                      " (CI: ", format( round( roc_ci[ 2, 1 ], digits = 2), nsmall = 2 ),
                                      "-", format( round( roc_ci[ 2, 3 ], digits = 2), nsmall = 2 ),
                                      ")"), x = 30, y = 5, size = 4, colour = "gray30" ),
  ...
  plots[[ 3 ]]$p + 
    ...
    theme( axis.title = element_text( size = 12, colour = "black" ), axis.text = element_text( size = 11, colour = "black" ) ) +
    coord_cartesian( xlim = c( 0, 220 ), ylim = c( 0, 65 ) ) +
    annotate( "text", label = paste0( "SD: ", plots[[ 3 ]]$sd ), x = 23, y = 56, size = 4, colour = "gray30" )+
    annotate( "text", label = paste0( "Cohen's d: ", format( plots[[ 3 ]]$cohens_d, nsmall = 2 ) ), x = 42, y = 52, size = 4, colour = "gray30" ),
  ...
  plots[[ 4 ]]$p + 
    ...
    theme( axis.title = element_text( size = 12, colour = "black" ), axis.text = element_text( size = 11, colour = "black" ) ) +
    coord_cartesian( xlim = c( 0, 220 ), ylim = c( 0, 65 ) ) +
    annotate( "text", label = paste0( "SD: ", plots[[ 4 ]]$sd ), x = 23, y = 56, size = 4, colour = "gray30" )+
    annotate( "text", label = paste0( "Cohen's d: ", format( round( plots[[ 4 ]]$cohens_d, digits = 2 ), nsmall = 2 ) ), x = 42, y = 52, size = 4, colour = "gray30" ),
  
  plots_roc[[ 4 ]]$p + 
    theme( legend.position = "none" ) +
    theme( axis.title = element_text( size = 12, colour = "black" ), axis.text = element_text( size = 11, colour = "black" ) ) +
    annotate( "text", label = paste0( "AUC: ", format( round( roc_ci[ 4, 2 ], digits = 2 ), nsmall = 2 ),
                                      " (CI: ", format( round( roc_ci[ 4, 1 ], digits = 2), nsmall = 2 ),
                                      "-", format( round( roc_ci[ 4, 3 ], digits = 2), nsmall = 2 ),
                                      ")"), x = 30, y = 5, size = 4, colour = "gray30" ),
...
# save plot
save_plot( p_comb_hist_roc_sd, file = paste0( outdir, "/FACET_hist_roc_sd.png" ), ncol = 2, nrow = 4, base_width = 4.5, base_height = 3.7 )
